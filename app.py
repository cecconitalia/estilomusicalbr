...
def atualizar_status_pedido(pedido_id):
    ...
    # Atualizar status de itens individuais se necessário
    for item in pedido.itens:
        item_status = request.form.get(f'item_status_{item.id}', item.status)
...
def ver_carrinho():
    ...
    total_com_desconto = total_sem_desconto - desconto_prime
...
def enviar_notificacao_admin(pedido):
    try:
        ...
        for admin in admins:
            # Registrar no log (substitua por email/WhatsApp/etc)
            app.logger.info(
                f"Notificação para admin {admin.email}: "
                f"Novo pedido #{pedido.id} via {pedido.metodo_pagamento}\n"
                f"Cliente: {pedido.cliente_nome} ({pedido.cliente_email})\n"
        ...
        app.logger.error(f"Erro ao enviar notificação para admin: {str(e)}")
...
def finalizar_pedido():
    """Finaliza o pedido e aplica descontos Prime"""
    try:
        ...
        # Calcular totais com desconto Prime
        total_sem_desconto = sum(float(item['preco']) * int(item['quantidade']) for item in session['carrinho'].values())
        
        # Aplicar desconto Prime se disponível
        desconto_prime = 0.0
        ...
        # Registrar comissão para o indicador, se aplicável
        if (hasattr(current_user, 'indicador_id') and current_user.indicador_id and
            hasattr(current_user, 'desconto_prime') and current_user.desconto_prime > 0):
            
            # Buscar percentual de comissão do código Prime usado
            codigo_prime = PrimeCode.query.filter_by(codigo=current_user.prime_code).first()
            percentual_comissao = codigo_prime.percentual_comissao if codigo_prime else 0
            
            if percentual_comissao > 0:
                valor_comissao = total_sem_desconto * (percentual_comissao / 100)
                ...
                indicador = User.query.get(current_user.indicador_id)
                if indicador:
                    indicador.saldo_comissoes = (indicador.saldo_comissoes or 0) + valor_comissao
